...
//    private DashboardController dashboardController;
    ...
    public CompleteController() throws SQLException, ClassNotFoundException {
...
//        dashboardLoader = new FXMLLoader(getClass().getResource("/fxml/Dashboard.fxml"));
        try {
            ...
            miniTaskDataCellController.setCompleteController(this);
            //
//            dashboardLoader.load();
//            dashboardController = dashboardLoader.getController();
//            dashboardController.setCompleteController(this);
        } catch (IOException ex) {
            Logger.getLogger(CompleteController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void updateUI() {
        try {
            ...
            try {
                ...
                ResultSet rs = stmt.executeQuery("SELECT * FROM TaskDayListDB WHERE DATECREATED >= '" + checkDate + "' AND ISCOMPLETE = 1 AND TASKLOCATION = 0 ORDER BY DATECREATED ASC, TIMECREATED ASC, TIMESTAMPCREATED DESC");
                while (rs.next()) {
    ...
    public void editTask(LocalDate dateValue, TaskDataModel items) {
        try {
            ...
            editTaskController.setCompleteController(this);
            editTaskController.setEditTaskValue(items, 2);
        } catch (IOException ex) {
            Logger.getLogger(CompleteController.class
    ...
    public void sortBy(String sortParam, String sortMode) {
        try {
            ...
            try {
                ...
                Statement stmt = connection.createStatement();
                Date checkDate = Date.valueOf(dateConverter.toString(LocalDate.now()));
                ResultSet rs = stmt.executeQuery("SELECT * FROM TaskDayListDB WHERE DATECREATED >= '" + checkDate + "' AND ISCOMPLETE = 1 AND TASKLOCATION = 0 ORDER BY " + sortParam + " " + sortMode);
                while (rs.next()) {
                    int taskID = rs.getInt(1);
                    Date dateCreated = rs.getDate(2);
                    Time timeCreated = rs.getTime(3);
