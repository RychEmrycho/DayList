...
public class TrashController implements Initializable {
    ...
    @FXML
    private JFXButton deletePermanentBtn;
...
//    private DashboardController dashboardController;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        deletePermanentBtn.setOnAction(e -> {
            deletePermanentItem(taskListViewContainer.getSelectionModel().getSelectedItem().getTaskID());
            taskListViewContainer.getItems().removeAll(dataTaskObservableList);
            updateUI();
            resetUI();
            taskListViewContainer.setItems(dataTaskObservableList);
            taskListViewContainer.setCellFactory(dataTaskListView -> new MiniTaskDataListViewCell());
            selectFirstItem();
            snackbar.enqueue(new JFXSnackbar.SnackbarEvent("Item has been deleted permanently"));
        });

        //Edit Button
        recoverBtn.setOnAction(e -> {
            recoverItem(taskListViewContainer.getSelectionModel().getSelectedItem().getTaskID());
            taskListViewContainer.getItems().removeAll(dataTaskObservableList);
            updateUI();
            resetUI();
            taskListViewContainer.setItems(dataTaskObservableList);
            taskListViewContainer.setCellFactory(dataTaskListView -> new MiniTaskDataListViewCell());
            selectFirstItem();
            snackbar.enqueue(new JFXSnackbar.SnackbarEvent("Item has recovered"));
        });
...
//        dashboardLoader = new FXMLLoader(getClass().getResource("/fxml/Dashboard.fxml"));
        try {
            ...
            miniTaskDataCellController.setTrashController(this);
            //
//            dashboardLoader.load();
//            dashboardController = dashboardLoader.getController();
//            dashboardController.setTrashController(this);
        } catch (IOException ex) {
            Logger.getLogger(TrashController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void updateDetails() {
        ...
        deletePermanentBtn.setDisable(false);
    }
    ...
    private void updateUI() {
        try {
            ...
            try {
                ...
                ResultSet rs = stmt.executeQuery("SELECT * FROM TaskDayListDB WHERE TASKLOCATION = 1 ORDER BY DATECREATED ASC, TIMECREATED ASC, TIMESTAMPCREATED DESC");
                while (rs.next()) {
    ...
    public void deletePermanentItem(int taskID) {
        try {
            Class.forName("org.h2.Driver");
            try {
                Connection connection = DriverManager.getConnection("jdbc:h2:~/DaylistDB", "root", "root");
                Statement stmt = connection.createStatement();
                stmt.executeUpdate("DELETE FROM TaskDayListDB WHERE ID = " + taskID);
                stmt.close();
                connection.close();
            } catch (SQLException e) {
                System.out.println("SQLExeption: " + e.getMessage());
    ...
    public void recoverItem(int taskID) {
        try {
            Class.forName("org.h2.Driver");
            try {
                Connection connection = DriverManager.getConnection("jdbc:h2:~/DaylistDB", "root", "root");
                Statement stmt = connection.createStatement();
                stmt.executeUpdate("UPDATE TaskDayListDB SET TASKLOCATION = 0 WHERE ID = " + taskID);
                stmt.close();
                connection.close();
            } catch (SQLException e) {
                System.out.println("SQLExeption: " + e.getMessage());
    ...
    public void sortBy(String sortParam, String sortMode) {
        try {
            ...
            try {
                ...
                Statement stmt = connection.createStatement();
                Date checkDate = Date.valueOf(dateConverter.toString(LocalDate.now()));
                ResultSet rs = stmt.executeQuery("SELECT * FROM TaskDayListDB WHERE TASKLOCATION = 1 ORDER BY " + sortParam + " " + sortMode);
                while (rs.next()) {
                    int taskID = rs.getInt(1);
                    Date dateCreated = rs.getDate(2);
                    Time timeCreated = rs.getTime(3);
    ...
    public void resetUI() {
        ...
        deletePermanentBtn.setDisable(true);
    }
}
